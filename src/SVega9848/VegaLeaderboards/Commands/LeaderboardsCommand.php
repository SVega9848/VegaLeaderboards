...
class LeaderboardsCommand extends Command implements PluginOwned {

    private Loader $loader;

    public function __construct() {
        parent::__construct("vegaleaderboards", "Display leaderboards on your server!", "", ["ldb", "leaderboards"]);
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) {
        ...
        if ($sender instanceof Player) {
            if($sender->hasPermission("vegaleaderboards.cmd")) {
                if(isset($args[0])) {
                    switch($args[0]) {
                        case "help":
